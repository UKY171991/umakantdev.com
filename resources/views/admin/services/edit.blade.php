...
<div class="row">
            ...
            <form action="{{ route('admin.services.update', $service) }}" method="POST">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        @enderror
                        <small class="form-text text-muted">Current slug: {{ $service->slug }}</small>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @error('service_category_id') is-invalid @enderror" 
                            ...
                            @foreach($categories as $id => $name)
                                <option value="{{ $id }}" {{ old('service_category_id', $service->service_category_id) == $id ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('content') is-invalid @enderror" 
                                  id="content" name="content" rows="8">{{ old('content', $service->content) }}</textarea>
                        @error('content')
                            <span class="invalid-feedback" role="alert">{{ $message }}</span>
                        @enderror
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control @error('price') is-invalid @enderror" 
                                       id="price" name="price" value="{{ old('price', $service->price) }}" step="0.01" min="0">
                                @error('price')
                                    <span class="invalid-feedback" role="alert">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select class="form-control @error('price_type') is-invalid @enderror" 
                                    ...
                                    <option value="project" {{ old('price_type', $service->price_type) == 'project' ? 'selected' : '' }}>Project Based</option>
                                </select>
                                @error('price_type')
                                    <span class="invalid-feedback" role="alert">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <div class="icheck-warning">
                                    <input type="checkbox" id="is_featured" name="is_featured" value="1" {{ old('is_featured', $service->is_featured) ? 'checked' : '' }}>
    ...
    <div class="col-md-4">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Service Stats</h3>
            </div>
            <div class="card-body">
                <div class="form-group">
                    <label><strong>Category:</strong></label>
                    <p>{{ $service->category ? $service->category->name : 'No Category' }}</p>
                </div>
                
                <div class="form-group">
                    <label><strong>Current Price:</strong></label>
                    <p>{{ $service->formatted_price }} ({{ $service->price_type }})</p>
                </div>
                ...
                <div class="form-group">
                    ...
                    <p>{{ $service->created_at->format('M j, Y g:i A') }}</p>
                </div>
                
                <div class="form-group">
                    <label><strong>Last Updated:</strong></label>
                    <p>{{ $service->updated_at->format('M j, Y g:i A') }}</p>
                </div>
            </div>
        </div>
    </div>
